"""CERES-Wheat root growth and water uptake.

Wheat roots are shallower and denser than maize roots.  The same
Ritchie-type extraction function is used, with a slower depth extension
rate (2.5 cm per °C·d vs 4.5 for maize).

Adapted from the maize root module (``MZ_ROOTS.for``) with wheat-specific
parameters.
"""
...
def grow_roots(
    ...
    """Extend rooting depth and distribute root length density for wheat.
    ...
    """
    # Wheat root depth extension: ~2.5 cm per °C·d
    drtdep = dtt * 2.5 / 100.0
...
def root_water_uptake(
    ...
    """Compute potential root water uptake by layer (Ritchie method).
    ...
    Returns:
        Tuple ``(rwu, trwup)`` where ``rwu`` is the per-layer extraction
        array and ``trwup`` is the total potential root water uptake (cm d⁻¹).
    """
    rwu = np.zeros(NL)
    trwup = 0.0
    ep1 = eop * 0.1  # mm → cm
